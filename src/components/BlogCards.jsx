        ...
        <div className='grid md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-8'>
            {
                filteredBlogs.map((blog) => <Link to={`/blogs/${blog.id}`} key={blog.id} className='p-5 shadow-lg rounded cursor-pointer'>
                    ...
                    <h3 className='mt-4 mb-2 font-bold hover:text-zinc-400 cursor-pointer'>{blog.title}</h3>
                    <p className='mb-2 text-gray-600 text-sm'><FaUser className='inline-flex items-center mr-2' />{blog.author}</p>
